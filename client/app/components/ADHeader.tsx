...
const ADHeader:React.FC<ADHeaderProps> = (props) => {
  const { isADAuthenticated, userDetails, adLogout } = useContext(AuthContext);

  const [isOpen, setIsOpen] = useState(false);

  const [userD, setUserD] = useState(false);

  const [isLoggedIn, setIsLoggedIn] = useState(isADAuthenticated());
  ...
  return (
    <>
      <header className={`main_header  ${headerClass}`}>
        <Navbar expand="md">
          <div className={props.widthType === 'full'? 'container-fluid' : 'container'}>
